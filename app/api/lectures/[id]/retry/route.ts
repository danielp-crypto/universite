...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!DEEPGRAM_API_KEY || !NEXT_PUBLIC_SITE_URL || !DEEPGRAM_WEBHOOK_SECRET) return NextResponse.json({ success: false, error: 'server_configuration_error' }, { status: 500 });

    const { id: lectureId } = await params;
    const { data: lecture, error: fetchError } = await supabaseAdmin.from('lectures')
      .select('id, user_id, file_path, status, processing_attempts, mime_type').eq('id', lectureId).single();
    if (fetchError || !lecture) return NextResponse.json({ success: false, error: 'lecture_not_found' }, { status: 404 });
    if (lecture.user_id !== user.id) return NextResponse.json({ success: false, error: 'forbidden' }, { status: 403 });
    if (lecture.status !== 'failed') return NextResponse.json({ success: false, error: 'not_failed' }, { status: 400 });
    if ((lecture.processing_attempts || 0) >= MAX_PROCESSING_ATTEMPTS) return NextResponse.json({ success: false, error: 'retry_limit_reached' }, { status: 429 });
    if (!lecture.file_path) return NextResponse.json({ success: false, error: 'file_unavailable' }, { status: 409 });

    const { data: signedUrlData, error: signedUrlError } = await supabaseAdmin.storage.from('lecture-media').createSignedUrl(lecture.file_path, SIGNED_URL_EXPIRY_SECONDS);
    if (signedUrlError || !signedUrlData?.signedUrl) return NextResponse.json({ success: false, error: 'file_unavailable' }, { status: 409 });

    const nextAttempt = (lecture.processing_attempts || 0) + 1;
    const { error: resetError } = await supabaseAdmin.from('lectures').update({
      status: 'processing', transcription_status: 'processing', transcription_error: null,
      processing_error_code: null, processing_attempts: nextAttempt,
      transcription_started_at: new Date().toISOString(), transcription_completed_at: null, transcription_failed_at: null,
    }).eq('id', lectureId).eq('status', 'failed');
    if (resetError) {
    ...
    try {
      ...
      await supabaseAdmin.from('deepgram_webhook_logs').insert({ lecture_id: lectureId, outcome: 'retry_submitted', raw_payload: accepted });
...
async function markRetryFailed(lectureId: string, errorCode: string, reason: string) {
  try {
    ...
    await supabaseAdmin.from('deepgram_webhook_logs').insert({ lecture_id: lectureId, outcome: 'retry_submission_failed', error: reason });
  } catch (err) {
