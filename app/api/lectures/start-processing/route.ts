...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ALLOWED_MIME_TYPES.has(normalizedMimeType)) {
      return NextResponse.json({ success: false, error: 'unsupported_audio_type' }, { status: 415 });
    }
    ...
    // Storage objects must live inside the authenticated user's namespace.
    if (!String(file_path).startsWith(`${user.id}/`)) {
      return NextResponse.json({ success: false, error: 'invalid_file_path' }, { status: 403 });
    }

    // Only one Deepgram job per student at a time for the MVP. This prevents
    // accidental double-clicks and avoids several long jobs consuming quota.
    ...
    if ((processingCount || 0) >= 1) {
      return NextResponse.json({ success: false, error: 'processing_limit_reached' }, { status: 409 });
    }
    ...
    const { data: signedUrlData, error: signedUrlError } = await supabaseAdmin
      .storage.from('lecture-media').createSignedUrl(file_path, SIGNED_URL_EXPIRY_SECONDS);
